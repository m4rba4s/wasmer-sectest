...
cockpit, text output, Markdown, JSON, or SARIF.
...
Inside the menu, choose a scenario, press Enter to return after it finishes, or
type `q` to close the application. Menu runs are recorded under
`target/security-sessions/`; the console can show recent session history,
preview the latest JSON evidence file, explain one case, or export an interview
bundle.
...
- `src/report.rs`: text, Markdown, JSON, and SARIF evidence output.
